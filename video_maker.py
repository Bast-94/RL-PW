...
out = cv2.VideoWriter('output/output_video.avi',cv2.VideoWriter_fourcc(*'DIVX'), 60, frameSize)
t_max = int(1e3)
q_learning_agent = QLearningAgent(
    learning_rate=0.5, epsilon=0.1, gamma=0.99, legal_actions=list(range(n_actions))
)
q_learning_eps_agent = QLearningAgentEpsScheduling(
    learning_rate=0.5, epsilon=0.1, gamma=0.99, legal_actions=list(range(n_actions))
)
sarsa_agent = SarsaAgent(learning_rate=0.5, gamma=0.99, legal_actions=list(range(n_actions))) 

agents = [q_learning_agent, q_learning_eps_agent, sarsa_agent]
agent_names = ["Q-learning", "Q-learning with eps scheduling", "Sarsa"]
